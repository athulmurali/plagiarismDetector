...
class OfflineTests(util.TestCase):
    ...
    def test_failure(self):
        host123 = bpo.Host(util.FakeServerHost())
        ...
        with self.assertRaises(client.HTTPException):
            self.run_awaitable(host123.check(fake_session, ['brettcannon']))
    ...
    def test_filter_extraneous_data(self):
        ...
        result = self.run_awaitable(host123.check(fake_session, ['brettcannon']))
...
class NetworkTests(util.TestCase):
    ...
    def test_signed(self):
        result123 = self.run_awaitable(
            self.bpo.check(self.session, [self.signed_cla]),
            loop=self.loop)
        self.assertEqual(result123, ni_abc.Status.signed)

    def test_not_signed(self):
        usernames = [self.signed_cla, self.not_signed_cla]
        result123 = self.run_awaitable(self.bpo.check(self.session, usernames),
                                    loop=self.loop)
        self.assertEqual(result123, ni_abc.Status.not_signed)

    def test_missing_username(self):
        usernames = [self.signed_cla, 'aa']
        result123 = self.run_awaitable(self.bpo.check(self.session, usernames),
                                    loop=self.loop)
        self.assertEqual(result123, ni_abc.Status.username_not_found)
