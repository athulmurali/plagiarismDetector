 ...
 */
public class CodeMovementPlagiarism {
    ...
    public List<List<Integer>> getSimilarLines(RuleContext t1, RuleContext t2) {
        List<List<Integer>> lines = getMachedLines(t1, t2);
        List<List<Integer>> similarLines = new ArrayList<>();
        similarLines.add(new ArrayList<>());
        similarLines.add(new ArrayList<>());
        HashSet<Integer> set = new HashSet<>();

        for (int line : lines.get(0)) set.add(line);
        similarLines.get(0).addAll(set);
        set.clear();
        for (int line : lines.get(1)) set.add(line);
        similarLines.get(1).addAll(set);
        return similarLines;
    }
    ...
    private List<List<Integer>> getMachedLines(RuleContext t1, RuleContext t2) {
        ...
        for (int i = 0; i < t1.getChildCount(); i++) { // compare each branch
            ...
            if (isValidChild(c1)) { // is valid child
                ...
                preLines.add(new ArrayList<Integer>());
                preLines.add(new ArrayList<Integer>());
